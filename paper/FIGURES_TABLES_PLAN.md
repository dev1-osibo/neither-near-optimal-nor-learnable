...
## Figures (9 files in `paper/figures/`)
...
| 7 | fig_sizing_sensitivity.png | §5.8 | sizing_sensitivity_lp.json → make_paper_figures.py | **READY** (RB→offline-optimum gap) |
| 8 | fig_learning_curves.png | §5.9 | checkpoint_curve in worker JSONs → make_learning_curves.py | **READY** (plateau by 1M steps; <0.7% over final 200k) |

*Note:* the headline figure is `fig_optimality_gap.png` (distance above the offline LP optimum).
...
| Ib | Distance above the **offline LP optimum** (RB→opt, RL→opt, vs old quantile-oracle gap) | §5.2 | **FINAL** (renewable 6–9%; storage 20–25%) |
...
| IV | Sizing sensitivity — RB→**offline-optimum** gap by battery capacity | §5.8 | **FINAL** (14–25% across 10/20/40 MWh; quantile-oracle contrast column) |
...
8. **Learning curves** (Fig 8): `python src/make_learning_curves.py` (reads `checkpoint_curve` in the main worker JSONs → `fig_learning_curves.png`; no retraining)
