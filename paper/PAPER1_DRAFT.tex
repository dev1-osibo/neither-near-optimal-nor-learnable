...
controller. I construct a leakage-free, real-data benchmark for a single
facility whose IT load is a replayed real workload trace (Alibaba
...
foresight, under fixed cooling), which I use in place of the weak
...
resolved, are physically small (\textasciitilde1\%). I conclude that the
standard framing is backwards: a well-tuned heuristic is a strong
...
conventional control. I revisit that claim for the single-facility case,
and I do so under an evaluation designed to remove the two shortcuts
that most often inflate RL results in this field.
...
I remove both. Training uses only episodes that begin and end before a
...
solely as a labeled upper bound. I then measure what advantage, if any,
survives.
...
This work provides a rigorous, real-data validation of the operational
core of the author's earlier vision paper (Osibo 2025), using only real,
causal, and reproducible inputs.
...
et al.~2018 for data-center cooling) are standard components I reuse as
baselines rather than as contributions.

\emph{Single-site on-site-asset work adjacent to this study.} A recent
cluster of papers studies on-site generation and storage at a single
data center but along different axes, and I distinguish each. Rafique et
al.~(2026, \emph{Energies}) design a hybrid MILP-plus-RL controller for
multi-zonal data centers with renewables and battery, reporting the
hybrid beats an uncertainty-aware MILP by up to \textasciitilde33\%;
their question is \emph{controller design} (RL vs MILP), whereas this
study is a \emph{benchmark} evaluation (RL vs a well-tuned heuristic,
both measured against an offline LP optimum), and my finding is the
opposite in spirit --- RL does not reliably beat the rule. Abdelhady et
al.~(2026, \emph{Applied Energy}) develop a regret-minimization
framework for \emph{strategic portfolio investment} in on-site solar,
wind, gas, and battery for hyperscale sites, showing diversified
portfolios dominate grid-only ones; that is an asset-sizing problem at a
multi-year horizon, while I study hour-by-hour operational dispatch of
an already-sized asset mix. Figini \& Paolone (2025, arXiv:2412.13853)
and Iqbal \& Sarwat (2026, \emph{IEEE Access}) each solve a
\emph{sizing} or \emph{reliability-under-outage} problem for a single
storage/PV stack --- the former carbon/cost-aware PV+ESS sizing, the
latter behind-the-meter BESS dispatch for critical-load continuity under
...
precisely the non-stationary setting I defer to a companion study
...
works investigate my seven axes \emph{separately}; none integrates them
...
\textbf{leakage-free, oracle-free} evaluation. My contribution is not a
...
I model the facility as an hourly discrete-time control problem over
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 1}]{figures/fig1_system_schematic.png}}
...
CO₂/kWh). I study the \textbf{twelve} source configurations obtained by
...
trace I reconstruct hourly cluster utilization by replaying instance
...
and day-of-week are real, calendar dates are not), I collapse the
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 2}]{figures/fig_load_typical_week.png}}
...
Per hour I accumulate three physical costs from the dispatch above:
...
as the RL agents, so the comparison is like-for-like. I define them
...
\item
  ...
  foresight) --- for each episode I solve a linear program that
  ...
  fixes the cooling lever and assumes perfect foresight, I call it the
  ...
  is multi-objective; I compare on \textbf{cost} because cost carries
  ...
  to approach, so the gap I attribute to a learner is if anything
...
I train \textbf{PPO} (Schulman et al.~2017), \textbf{SAC} (Haarnoja et
...
results in this literature, so I forgo it and report the untuned
...
than a pure algorithmic property. I report A2C as-is for completeness
...
scope here and are the most important learned-control extension I flag
...
I train \textbf{240 policies} (4 algorithms × 12 configurations × 5
...
8000--8199}, so all methods are scored on identical weeks. I compare
...
d ≈ 0.84 accompanies a 0.51\% cost change); I therefore treat the
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 3}]{figures/fig_optimality_gap.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 4}]{figures/fig_verdict_delta.png}}
...
Table I shows my MPC underperforming RuleBased everywhere, by +0.4\% in
...
\emph{naive-forecast} MPC, not a competent one --- I include it only to
show that bolting a controller onto a zero-information forecast buys
nothing. I therefore read the MPC row as a \textbf{sanity floor, not as
evidence about the potential of competent forecast-driven control}, and
I do not draw any conclusion about MPC as a method from it. The fair
...
of forecast-driven optimization that I did not attempt to realize
...
the open question this benchmark is built to pose; I leave the
...
forecaster.'' I test this directly by training \textbf{both PPO and SAC}
(the two algorithms that produce the wins and the least-bad storage
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 5}]{figures/fig_foresight_premium.png}}
...
Although RL does not beat the heuristic on the scalar objective, I ask a
distinct question: when RL \emph{is} used, do its reward weights give an
operator predictable control over the cost/carbon/water tradeoff? I
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 6}]{figures/fig_pareto_cost_carbon.png}}
...
Is the large storage gap an artifact of the 20 MWh battery? I
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 7}]{figures/fig_sizing_sensitivity.png}}
...
were simply undertrained, I log a held-out evaluation at every 100k-step
checkpoint and inspect the full learning curve. Figure 8 plots
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 8}]{figures/fig_learning_curves.png}}
...
opens a large gap that no controller I tested closes.''
...
coupling --- is exactly where a learner should beat a myopic rule. I
...
My results also carry a methodological message, and I am careful to
...
inflates the apparent advantage: in my experiments, a leaky split
reported a larger RL edge that the temporal split removed. The second
shortcut, \textbf{oracle price foresight} in the observation, I tested
head-on (§5.6) and found it does \emph{not} help the learned controller
here (mean premium ≈0); so in my case its removal is not what drives the
verdict, but it remains bad practice because it flatters forecast-driven
controllers and is undeployable. The honest picture is thus doubly
robust: the advantage is small under a leakage-free split, and it does
not grow even with perfect foresight. Because these shortcuts are
widespread, some reported single-site RL gains are plausibly evaluation
artifacts rather than control skill. I therefore argue that leakage-free
splits, oracle-free observations, and a \emph{well-tuned heuristic
reported as the comparator} (not a strawman) should be the default
hygiene for this literature.
...
my feedforward learners handle worst (§4.3, §6.2): whether memory closes
the storage gap is the sharpest open question this benchmark poses. Each
targets a specific alternative explanation rather than the headline
finding, which the leakage-free protocol, the foresight ablation, and
the convergence curves already support for the feedforward policies
studied here.
...
\item
  ...
  cluster; utilization→power is GPU-TDP-based. I frame the facility as
...
\item
  ...
  objective's absolute level without touching the cost verdict. I regard
  the exact magnitude of the storage gap (20--25\%) as
  ...
  every episode.
\item
  \textbf{MPC ran on a persistence forecast.} Disclosed; this is a
...
\item
  ...
  agent choices, and I disclose them because they shape the storage
  ...
  I note that the auto-charging reduces the agent's \emph{control
...
question I leave to future work (§6.6).
...
that no controller I tested closes. Perfect foresight is worth that
...
Methodologically, I argue that leakage-free splits, oracle-free
